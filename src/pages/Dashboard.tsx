...
import { Shield, AlertTriangle, XCircle, Mail } from "lucide-react";
...
export default function Dashboard() {
  const history: HistoryItem[] = useMemo(() => JSON.parse(localStorage.getItem("phishguard_history") || "[]"), []);
  ...
  const stats = useMemo(() => {
    ...
    return { total: history.length, safe, suspicious, phishing };
  }, [history]);

  const pieData = [
    { name: "Safe", value: stats.safe || 1, color: "hsl(var(--safe))" },
  ...
  const barData = useMemo(() => {
    ...
    return days.map((day) => {
      const dayItems = history.filter((h) => h.date?.slice(0, 10) === day);
      return {
        day: day.slice(5),
        Safe: dayItems.filter((h) => h.classification === "Safe").length,
        Suspicious: dayItems.filter((h) => h.classification === "Suspicious").length,
        Phishing: dayItems.filter((h) => h.classification === "Phishing").length,
      };
    });
  }, [history]);

  const statCards = [
    { label: "Emails Scanned", value: stats.total, icon: Mail, color: "neon-text" },
    { label: "Safe Emails", value: stats.safe, icon: Shield, color: "text-safe" },
    { label: "Suspicious", value: stats.suspicious, icon: AlertTriangle, color: "text-suspicious" },
    { label: "Phishing Detected", value: stats.phishing, icon: XCircle, color: "text-destructive" },
  ];
